          ...
          <div class="col-7">
            ...
            <div class="sales_and_reviews">
              ...
              <div class="rating" title="评分 {{ $product->rating }}">评分
                <span class="count">{{ str_repeat('★', floor($product->rating)) }}{{ str_repeat('☆', 5 -
                  floor($product->rating)) }}</span>
              </div>
            </div>
            <!-- 原普通商品模块结束 -->
            @endif
            ...
            <div class="sales_and_reviews">
              <div class="sold_count">累计销量 <span class="count">{{ $product->sold_count }}</span></div>
              <div class="review_count">累计评价 <span class="count">{{ $product->review_count }}</span></div>
              <div class="rating" title="评分 {{ $product->rating }}">评分 <span class="count">{{ str_repeat('★',
                  floor($product->rating)) }}{{ str_repeat('☆', 5 - floor($product->rating)) }}</span></div>
            </div> --}}
            <div class="skus">
              <label>选择</label>
   ...
   // 监听收藏按钮的点击事件
    ...
    $('.btn-add-to-cart').click(function () {
      ...
      axios.post('{{ route('cart.add') }}', {
        ...
        // sku_id: $('sku-btn input[name=sku]:checked').val(),
